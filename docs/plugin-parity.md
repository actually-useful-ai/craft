...
|---|---|---|
| Craft | [`actually-useful-ai/craft`](https://github.com/actually-useful-ai/craft) | `v0.3.0` |
...
install_plugin_release actually-useful-ai/craft v0.3.0 craft
...
The verifier reads each `.codex-plugin/plugin.json` version and hashes every `skills/**/SKILL.md` file in a stable order. It prints one row per host and plugin:

- `REFERENCE`: Neo's installed copy, used as the comparison source;
- `OK`: version and skill hash match Neo;
...
For fixture-based testing, use `scripts/plugin-parity.sh --fixture path/to/matrix.tsv`. The fixture format is tab-separated: host, plugin, version, and skill hash.
