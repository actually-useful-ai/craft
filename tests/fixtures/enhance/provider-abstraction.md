...
## Handoff
Hand to /craft:compose flow: implement a small service-owned Protocol and two adapters; do not reuse the missing shared module or add `geepers-kernel` by default. First confirm Python >=3.8 and consult the providers’ official SDK documentation; add fake-provider contract tests and adapter tests. Finish with /craft:reconsider --validate.
