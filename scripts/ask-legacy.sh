#!/usr/bin/env bash
# Explicit OpenAI compatibility transport for Swarm and legacy callers.
...
route() {
  ...
  case "$1" in
    ...
    openai|gpt)
      ...
      DIRECT_URL=https://api.openai.com/v1/chat/completions; DIRECT_KEY="${OPENAI_API_KEY:-}"
      ;;
    *) die "unknown provider '$1' (supported: luna, openai)" ;;
  esac
...
usage() {
  cat <<'EOF'
Usage:
  ask.sh [luna|openai] QUESTION
  ask.sh [luna|openai] -        # read question from stdin
  ...
  case "$1" in
    ...
    *)
      if [ -z "$PROVIDER_ALIAS" ]; then
        case "$1" in luna|openai|gpt) PROVIDER_ALIAS="$1"; shift; continue ;; esac
      fi
      break
      ;;
  esac
